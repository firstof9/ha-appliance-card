...
export class ApplianceCardEditor extends LitElement {
  ...
  private _schema() {
    ...
    const integrations = ['smartthings', 'localthings', 'smartthinq_sensors', 'lg_thinq', 'homeconnect'];
  ...
  private _autofillConfig(config: ApplianceCardConfig): ApplianceCardConfig {
    ...
    // 3. Autofill logic supporting SmartThings, LocalThings, SmartThinQ Sensors, LG ThinQ, and Home Connect
    ...
    newConfig.job_state_entity = newConfig.job_state_entity || findEntity(['_job_state', '_running_state', '_cycle_state', '_pre_state', '_current_course', '_progress', '_cooking_mode', '_program_progress', '_selected_program', '_active_program']);
    newConfig.time_entity = newConfig.time_entity || findEntity(['_time_remaining', '_remaining_time', '_time_left', '_estimated_finish', '_total_time', '_program_finish_time'], 'sensor');
    newConfig.wifi_entity = newConfig.wifi_entity || findEntity(['_wifi', '_connectivity', '_ssid'], 'binary_sensor') || findEntity(['_ssid'], 'sensor');
    newConfig.lock_entity = newConfig.lock_entity || findEntity(['_lock', '_child_lock', '_door_lock', '_remote_start', '_remote_control']);
